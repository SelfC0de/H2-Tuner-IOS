...
struct HomeView: View {
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 0) {
                ...
                connectionOrb.padding(.top, 32)
                statusSection.padding(.top, 24)
                ipSection.padding(.top, 24).padding(.horizontal, 20)
                linkInputSection.padding(.top, 24).padding(.horizontal, 20)
                if settings.selectedServer != nil {
                    selectedServerCard.padding(.top, 16).padding(.horizontal, 20)
                }
                connectButton.padding(.top, 24).padding(.horizontal, 20).padding(.bottom, 40)
            }
        }
    }

    // MARK: - Header with animated title
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                ZStack {
                    Text("H2 Tuner")
                        ...
                        .shadow(color: Color(hex: "#7C5CFC").opacity(titleGlow ? 0.8 : 0.2), radius: titleGlow ? 12 : 4)
                        .onAppear {
    ...
    private var connectionOrb: some View {
        ZStack {
            ForEach(0..<3) { i in
                Circle()
                    .stroke(orbColor.opacity(0.06 - Double(i) * 0.015), lineWidth: 1)
                    ...
                    .animation(.easeInOut(duration: 1.5 + Double(i) * 0.3).repeatForever(autoreverses: true), value: pulseAnimation)
            }
            Circle()
                .fill(RadialGradient(colors: [orbColor.opacity(0.25), orbColor.opacity(0.05), Color.clear], center: .center, startRadius: 20, endRadius: 90))
                .frame(width: 180, height: 180)
            Circle()
                .fill(LinearGradient(colors: [orbColor.opacity(0.9), orbColor.opacity(0.6)], startPoint: .topLeading, endPoint: .bottomTrailing))
            ...
            Image(systemName: orbIcon)
                ...
                .animation(isAnimating ? .linear(duration: 2).repeatForever(autoreverses: false) : .default, value: rotationAnimation)
        }
        .onAppear {
            pulseAnimation = true; glowAnimation = true
    ...
    private var orbColor: Color { vpnManager.connectionState.color }
    ...
    private var ipSection: some View {
        HStack(spacing: 12) {
            ...
            IPCard(title: "VPN IP", info: vpnManager.vpnIP, icon: "lock.shield.fill",
    ...
    private var linkInputSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                HStack(spacing: 8) {
                    Image(systemName: "link").font(.system(size: 14)).foregroundColor(Color(hex: "#5CF0FC"))
                ...
                .background(RoundedRectangle(cornerRadius: 12).fill(Color(hex: "#1A1A2E"))
                    .overlay(RoundedRectangle(cornerRadius: 12)
                        .stroke(linkFocused ? Color(hex: "#5CF0FC").opacity(0.5) : Color(hex: "#2A2A3E"), lineWidth: 1)))

                Button { pasteAndParse() } label: {
                    Image(systemName: "doc.on.clipboard").font(.system(size: 16)).foregroundColor(Color(hex: "#5CF0FC"))
                        .frame(width: 44, height: 44).background(Color(hex: "#1A1A2E"))
                        .clipShape(RoundedRectangle(cornerRadius: 12))
                        .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color(hex: "#2A2A3E"), lineWidth: 1))
                }
                if !linkText.isEmpty {
                    Button { parseLink() } label: {
                        Image(systemName: "plus.circle.fill").font(.system(size: 16)).foregroundColor(Color(hex: "#5CFC8A"))
                            .frame(width: 44, height: 44).background(Color(hex: "#1A1A2E"))
                            .clipShape(RoundedRectangle(cornerRadius: 12))
                            .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color(hex: "#2A2A3E"), lineWidth: 1))
                    }
    ...
    private var selectedServerCard: some View {
        Group {
            if let server = settings.selectedServer {
                HStack(spacing: 12) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 10).fill(server.protocol.accentColor.opacity(0.15))
                        Image(systemName: server.protocol.icon).font(.system(size: 16, weight: .medium)).foregroundColor(server.protocol.accentColor)
                    }
                    .frame(width: 40, height: 40)
                    VStack(alignment: .leading, spacing: 2) {
                        Text(server.name).font(.system(size: 14, weight: .semibold)).foregroundColor(.white).lineLimit(1)
                        Text("\(server.protocol.displayName) · \(server.host):\(server.port)")
                            .font(.system(size: 11)).foregroundColor(Color(hex: "#8A9BB8"))
                    }
                    Spacer()
                    Image(systemName: "checkmark.circle.fill").font(.system(size: 16)).foregroundColor(server.protocol.accentColor)
                }
                .padding(14)
                .background(RoundedRectangle(cornerRadius: 14).fill(Color(hex: "#1A1A2E"))
                    .overlay(RoundedRectangle(cornerRadius: 14).stroke(server.protocol.accentColor.opacity(0.3), lineWidth: 1)))
            }
        }
    }
    ...
    private var connectButton: some View {
        Button { handleConnect() } label: {
            ...
            .background(RoundedRectangle(cornerRadius: 16).fill(connectButtonGradient)
...
struct IPCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 6) {
                Image(systemName: icon).font(.system(size: 11, weight: .semibold)).foregroundColor(accentColor)
                Text(title).font(.system(size: 11, weight: .semibold)).foregroundColor(Color(hex: "#8A9BB8"))
            }
            if isEmpty {
                Text("—").font(.system(size: 15, weight: .bold, design: .monospaced)).foregroundColor(Color(hex: "#444460"))
            } else if let info = info {
                Text(info.ip).font(.system(size: 14, weight: .bold, design: .monospaced)).foregroundColor(.white).lineLimit(1).minimumScaleFactor(0.7)
                if let country = info.country, let city = info.city {
                    Text("\(country) · \(city)").font(.system(size: 10)).foregroundColor(Color(hex: "#8A9BB8")).lineLimit(1)
                }
            } else {
                HStack(spacing: 4) {
                    ProgressView().progressViewStyle(CircularProgressViewStyle(tint: accentColor)).scaleEffect(0.6)
                    Text("Загрузка...").font(.system(size: 11)).foregroundColor(Color(hex: "#666680"))
                }
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading).padding(14)
        .background(RoundedRectangle(cornerRadius: 14).fill(Color(hex: "#1A1A2E"))
            .overlay(RoundedRectangle(cornerRadius: 14).stroke(accentColor.opacity(isEmpty ? 0.1 : 0.25), lineWidth: 1)))
    }
}
